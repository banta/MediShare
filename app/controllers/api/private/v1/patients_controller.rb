class Api::Private::V1::PatientsController < Api::Private::V1::BaseController
  ...
  def create
    ...
    @patient.health_facility_id = params[:health_facility_id]
  	...
  	if @patient.save
  		respond_with(@patient, :api_template => :v1_applicants_private, :status => :ok)
  	else
  ...
  def show
    ...
    respond_with(@applicant, :api_template => :v1_applicants_private, :status => :ok)
  end

  def test
    Mailer.test(params[:to], params[:subject], params[:body]).deliver!
    head :created
  end
end